package org.exoplatform.social.benches.util;
 ...
 * 3) -v means that this syllable can only added to another syllable, that ends with a vocal.
...
public class NameGenerator {
  ...
  public void changeSyllabes(String syllabes) {
    if(syllabes == null) throw new RuntimeException("Syllabes cannot be null");
    this.syllabes = syllabes;
    refresh();
  }
  ...
  public String compose(int syls){
    if(syls > 2 && mid.size() == 0) throw new RuntimeException("You are trying to create a name with more than 3 parts, which requires middle parts, " +
        "which you have none in the file "+syllabes+". You should add some. Every word, which doesn't have + or - for a prefix is counted as a middle part.");
