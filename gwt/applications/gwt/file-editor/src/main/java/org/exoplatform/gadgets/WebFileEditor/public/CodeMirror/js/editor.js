...
var Editor = (function(){
  ...
  function Editor(CodeMirror) {
    this.options = options = CodeMirror.options;
    this.parent = parent;
    this.doc = CodeMirror.win.document;
    this.container = this.doc.body;
    this.win = CodeMirror.win;
    ...
    if (!options.readOnly) {
      ...
      function setEditable(doc) {
        // In IE, designMode frames can not run any scripts, so we use
        // contentEditable instead.
        if (doc.body.contentEditable != undefined && /MSIE/.test(navigator.userAgent))
      ...
      try {
        setEditable(this.doc);
      }
      catch(e) {
        var focusEvent = addEventHandler(this.doc, "focus", function() {
          focusEvent();
          setEditable();
        }, true);
      }

      addEventHandler(this.doc, "keydown", method(this, "keyDown"));
      addEventHandler(this.doc, "keypress", method(this, "keyPress"));
      addEventHandler(this.doc, "keyup", method(this, "keyUp"));

      function cursorActivity() {self.cursorActivity(false);}
      addEventHandler(this.doc.body, "paste", cursorActivity);
      addEventHandler(this.doc.body, "cut", cursorActivity);
      addEventHandler(this.doc.body, "mouseup", cursorActivity);

      if (this.options.autoMatchParens)
        addEventHandler(this.doc.body, "click", method(this, "scheduleParenBlink"));
    }
  }
...
addEventHandler(window, "load", function() {
  var CodeMirror = window.editor.mirror.win.frameElement.CodeMirror;
  CodeMirror.editor = new Editor(CodeMirror);
