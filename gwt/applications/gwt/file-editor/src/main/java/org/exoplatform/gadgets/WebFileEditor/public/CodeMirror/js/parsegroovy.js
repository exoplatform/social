...
var GroovyParser = Editor.Parser = (function() {
  ...
  function GroovyLexical(indented, column, type, align, prev) {
    ...
    this.prev = prev;
	this.value = '';
	this.firstLineAfterBracket = true;
  }
  // My favourite Groovy indentation rules.
...
/* fixed error with initial lexical.indented of new line = 1 instead of 0 */    
      // Adjust column and indented.
		if (token.type === "whitespace" && token.content !== "\n") { 
			indented = lexical.indented = token.value.length;
		}
		
		if ( lexical.type == '}' && token.content != '\n' && lexical.firstLineAfterBracket ) {
			lexical.firstLineAfterBracket = false;
		}
      ...
      while(true) {
        ...
        if (consume){
          ...
          // Here we differentiate between local and global variables.
            ...
            token.style = "groovyLocalvariable";
          return token;
        }
      }
    }
    ...
    function statement(type){
	  if (type == "javaKeywordA") cont(pushlex("form"), expression, statement, poplex);
      else if (type == "javaKeywordB") cont(pushlex("form"), statement, poplex);
    ...
    function expression(type){
      ...
      else if (type == "javaKeywordC" || type == "groovyKeywordC") cont(expression);
    ...
    function property(type){
      if (type == "variable") {mark("groovyProperty"); cont();}
    }
    // This parses a property and its value in an object literal.
    function objprop(type){
      if (type == "variable") mark("groovyProperty");
    ...
    function forspec1(type, value){
      if (type == "javaType") cont(vardef1, forspec2);
