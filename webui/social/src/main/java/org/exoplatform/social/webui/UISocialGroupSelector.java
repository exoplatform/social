...
public class UISocialGroupSelector extends UIContainer
{
   ...
   public UISocialGroupSelector() throws Exception
   {
      ...
      UITree tree = addChild(UIFilterableTree.class, "UITreeGroupSelector", "TreeGroupSelector");
   ...
   private void setupFilterableTree() 
   {
	  UIFilterableTree.TreeNodeFilter nodeFilter = new UIFilterableTree.TreeNodeFilter() {
		...
		public boolean filterThisNode(Object nodeObject, WebuiRequestContext context) {
			...
			try {
				membership = orgService.getMembershipHandler().findMembershipByUserGroupAndType(remoteUser, group.getId(), MANAGER);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			try {
				...
				boolean existMembership = checkMembershipOfChildren(groups, orgService, remoteUser);
   ...
   static public class ChangeNodeActionListener extends EventListener<UIFilterableTree>
   {
      public void execute(Event<UIFilterableTree> event) throws Exception
      {
         UISocialGroupSelector uiGroupSelector = event.getSource().getAncestorOfType(UISocialGroupSelector.class);
