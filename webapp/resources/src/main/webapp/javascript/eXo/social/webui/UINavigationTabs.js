 ...
 * @since Social 1.2.8
 * //TODO implement animation for smoother item navigating
 */
...
(function(eXo) {
  ...
  function _showAndHideItems() {
    ...
    debug.info('uiTabWidth: ' + uiTabWidth);
    //min viewPort's width
    if (viewPort.width < 600) {
      calculatedDisplayElements = 3;
    } else {
      var addedMoreTabsNum = Math.floor((viewPort.width - 600) / (uiTabWidth + 20));
      calculatedDisplayElements = calculatedDisplayElements + addedMoreTabsNum;
    }

    if (configuration.displayedElements < calculatedDisplayElements) {
      calculatedDisplayElements = configuration.displayedElements;
    }
    debug.info('calculatedDisplayElements: ' + calculatedDisplayElements);
    ...
    if (configuration.pageScrolled) {
      ...
      if (lastVisibleIndex > tabElements.length - 1) {
        lastVisibleIndex = tabElements.length - 1;
      }
      //hide previous range
      ...
      //hide next range
      if (lastVisibleIndex < tabElements.length - 1) {
        _hideRange(lastVisibleIndex + 1, tabElements.length -1);
      }
    } else {
      selectedTabIndex = _getSelectedTabIndex();

      if ((selectedTabIndex + calculatedDisplayElements) > (tabElements.length - 1)) {
        selectedTabIndex = tabElements.length - calculatedDisplayElements;
      }
      ...
      if (lastShownTabIndex > tabElements.length - 1) {
        lastShownTabIndex = tabElements.length - 1;
      }
      ...
      _hideRange(lastShownTabIndex + 1, tabElements.length - 1);
    }
  }
  ...
  function _initNavigationButtons() {
   ...
   Util.addEventListener(nextButton, ['mousedown', 'keydown'], function() {
    ...
    if (!_previousButtonShown()) {
      ...
      previousButton.style.left = (firstVisiblePosition['x'] - 20) + 'px';
    }
    ...
    if (!_nextButtonShown()) {
      ...
      nextButton.style.left = (rightLastVisiblePositionX - 10) + 'px';
    }
  ...
  function _getSelectedTabIndex() {
    ...
    if (selectedTabIndex < 0) {
      debug.warn('no selectedTabEl found, return default value: 0');
      selectedTabIndex = 0;
    }
  ...
  function _setConfiguration(configurationParams) {
    configuration = eXo.extend(configuration, configurationParams);
    _init();
  ...
  function _isValidConfiguration() {
    if (!tabContainerEl) {
      debug.error('invalid configuration: tabContainerEl is null');
      return false;
    }
    return true;
  }
  ...
  //exposes
  eXo.provide("eXo.social.webui.UINavigationTabs", UINavigationTabs);
})(eXo);
