...
public class IdentityStorageTest extends AbstractCoreTest {
  ...
  public void testGetIdentitiesByFirstCharacterOfNameCount() throws Exception {
    populateData();
    int idsCount = identityStorage.getIdentitiesByFirstCharacterOfNameCount("organization", 'F');
    assertEquals("Number of identity must be " + idsCount, 10, idsCount);
    idsCount = identityStorage.getIdentitiesByFirstCharacterOfNameCount("organization", 'L');
  ...
  public void testGetIdentitiesByFirstCharacterOfName() throws Exception {
    populateData();
    assertEquals(1, identityStorage.getIdentitiesByFirstCharacterOfName("organization", 'F', 0, 1, false).size());
    assertEquals(10, identityStorage.getIdentitiesByFirstCharacterOfName("organization", 'F', 0, 10, false).size());
    assertEquals(0, identityStorage.getIdentitiesByFirstCharacterOfName("organization", 'L', 0, 10, false).size());
  }
