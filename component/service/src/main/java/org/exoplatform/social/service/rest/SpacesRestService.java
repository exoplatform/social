...
public class SpacesRestService implements ResourceContainer {
  ...
  /**
   * Gets a set of information of the target space. The returned information of the space includes display Name, member count
   * avatar, isOnlyManger and status with the current user who sends request (Member, Manger..).
   *
   * @param uriInfo The requested URI information.
   * @param securityContext The security context of the system.
   * @param spaceId The Id of a specific user.
   * @param format The format of the returned result, for example, JSON, or XML.
   ...
   * @return The detailed information of a space.
   * @throws Exception
   * @LevelAPI Platform
   * @anchor SpacesRestService.getSpaceInfo
   */
  @GET
  @Path("/getSpaceInfo/{spaceId}.{format}")
  public Response getSpaceInfo(@Context UriInfo uriInfo,
                               @PathParam("portalName") String portalName,
    ...
    if (format.indexOf('.') > 0) {
      spaceId = new StringBuffer(spaceId).append(".").append(format.substring(0, format.lastIndexOf('.'))).toString();
    ...
    Space space = _spaceService.getSpaceByPrettyName(spaceId);
    SpaceInfo spaceInfo = new SpaceInfo();

    // Process action
    if(currentUserName != null && updatedType != null) {

      if (Leave_ACTION.equals(updatedType)) {
        _spaceService.removeMember(space, currentUserName);
      } else if (Request_ACTION.equals(updatedType)) {
        _spaceService.addPendingUser(space, currentUserName);
      } else if(Cancel_ACTION.equals(updatedType)){
        _spaceService.removePendingUser(space, currentUserName);
      }
    }

    spaceInfo.setSpaceMemberCount(space.getMembers().length);
    spaceInfo.setDisplayName(space.getDisplayName());
    if (StringUtils.isNotEmpty(space.getAvatarUrl())) {
      spaceInfo.setAvatarURL(space.getAvatarUrl());
    } else {
      spaceInfo.setAvatarURL(LinkProvider.SPACE_DEFAULT_AVATAR_URL);
    }
    if(StringUtils.isNotEmpty(space.getDescription())){
      spaceInfo.setSpaceDescription(StringEscapeUtils.unescapeHtml(space.getDescription()));
    }
    spaceInfo.setStatus(getTypeOfSpace(space,currentUserName));
    spaceInfo.setIsOnlyManger(_spaceService.isOnlyManager(space, currentUserName));
  ...
  static public class SpaceInfo {
    ...
    public int getSpaceMemberCount() {
      return spaceMemberCount;
    }

    public void setSpaceMemberCount(int spaceMemberCount) {
    ...
    public boolean isOnlyManger() {
      return isOnlyManger;
    }

    public void setIsOnlyManger(boolean isOnlyManger) {
      this.isOnlyManger = isOnlyManger;
    }
  }
   ...
   * @return
   */
  private  String getTypeOfSpace(Space space, String currentUserId) {

    if (_spaceService.isInvitedUser(space, currentUserId)) { // Received
      return TypeOfSpace.INVITED.toString();
    } else if (_spaceService.isPendingUser(space, currentUserId)) { // Sent
      return TypeOfSpace.SENT.toString();
    } else if (_spaceService.isMember(space, currentUserId)) { // Member
      if (_spaceService.isManager(space, currentUserId)) {
