...
public class EntityBuilder {
  ...
  /**
   * Get a ActivityEntity from an activity in order to build a json object for the
   * rest service
   * 
  ...
  public static DataEntity getActivityStream(ExoSocialActivity activity, Identity authentiatedUsed) {
    ...
    if (owner != null) { // case of user activity
      ...
      if (!authentiatedUsed.getId().equals(activity.getPosterId()) // the viewer is not the poster
          ...
          && (relationship == null || !relationship.getStatus().equals(Relationship.Type.CONFIRMED)) // the viewer has no
      ...
      if (space == null || !(spaceService.isSuperManager(authentiatedUsed.getRemoteId())
          || spaceService.isMember(space, authentiatedUsed.getRemoteId()))) { // the viewer is not member of space
        return null;
      }
