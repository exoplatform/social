...
public class PeopleRestService implements ResourceContainer{
  ...
  /**
   * Gets a set of informations of the activity's likers users. The returned information of the user includes full name, position
   * avatar, link to profile and relationship status with the current user who sends request.
   *
   * @param uriInfo
  ...
  @GET
  @Path("/getLikersInfo/{activityId}.{format}")
  public Response getLikersInfo(@Context UriInfo uriInfo,
                                @Context SecurityContext securityContext,
    ...
    format = ArrayUtils.contains(mediaTypes, format) ? format : mediaTypes[0];
    MediaType mediaType = Util.getMediaType(format, mediaTypes);
    if (activity != null && activity.getLikeIdentityIds() != null) {
      for (String likerId : activity.getLikeIdentityIds()){
        ...
        if (identity != null) {
          ...
          if (!isAnonymous) { // private information
            ...
            if(currentUserName != null && !likerId.equals(currentUserName)) {
              // Set relationship type
              Relationship relationship = getRelationshipManager().get(currentIdentity, identity);
    ...
    return Util.getResponse(peopleInfos, uriInfo, mediaType, Response.Status.OK);
  }
