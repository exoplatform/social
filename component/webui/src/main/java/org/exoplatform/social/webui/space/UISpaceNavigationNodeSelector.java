      ...
      public void execute(Event<UIRightClickPopupMenu> event) throws Exception {
          ...
          for (PageNode pageNode : pageNodes) {
  			    if(pageNode != null && pageNode.isSystem()) {
  				    uiApp.addMessage(new ApplicationMessage("UISpaceNavigationNodeSelector.msg.systemnode-move", null, ApplicationMessage.ERROR));
  				    return;
  			    }
          }
   ...
   static public class DeleteNodeActionListener extends EventListener<UIRightClickPopupMenu> {
      public void execute(Event<UIRightClickPopupMenu> event) throws Exception {
         ...
         if(childNode.isSystem()) {
        		uiApp.addMessage(new ApplicationMessage("UISpaceNavigationNodeSelector.msg.systemnode-delete", null, ApplicationMessage.ERROR));
        		return;
        	}
